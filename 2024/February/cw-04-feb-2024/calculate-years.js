...
// has to be kept in the bank in order for it to amount to a desired 
// sum of money 'D'.

// The sum is kept for 'Y' years in the bank where interest 'I' is 
// paid yearly. After paying taxes 'T' for the year the new sum is 
// re-invested.
...
// number of years 'Y' as a whole in order for Mr. Scrooge to get 
// the desired sum.

// Assumption: Assume that Desired Principal 'D' is always greater 
// than the initial principal. However it is best to take into 
// consideration that if Desired Principal 'D' is equal to Principal 
...
function calculateYears(principal, interest, tax, desired) {
    ...
    while(currentAmount < desired){
        ...
        let interestEarned = currentAmount * interest;
        let taxPaid = interestEarned * tax;
        let earnings = interestEarned - taxPaid;
        currentAmount = currentAmount + earnings;
        years++;
    }
